...
def prepare_fp8_moe_layer_for_marlin(
    ...
    orig_n = n
    orig_w13_n = w13_n
    assert w13_n % n == 0, "w13 gate/up shards must align with intermediate size"
    w13_shards = w13_n // n
    n_pad = _round_up(n, _MARLIN_MOE_N_PAD_MULTIPLE) - n
    if n_pad:
        if weight_block_size is not None:
            raise RuntimeError(
                "FP8 MoE Marlin N-padding is only implemented for tensor/channel "
                "W8A16 scales; block-FP8 scales need scale-row padding too."
            )

        padded_n = n + n_pad
        w13_weight = w13_weight.reshape(e, w13_shards, n, k)
        w13_weight = torch.nn.functional.pad(w13_weight, (0, 0, 0, n_pad))
        w13_weight = w13_weight.reshape(e, w13_shards * padded_n, k).contiguous()
        w2_weight = torch.nn.functional.pad(w2_weight, (0, n_pad)).contiguous()

        layer._marlin_moe_orig_intermediate_size_per_partition = n
        layer._marlin_moe_n_pad = n_pad
        layer.intermediate_size_per_partition = padded_n
        n = padded_n
        w13_n = w13_weight.size(1)
        logger.warning_once(
            "Padding FP8 MoE Marlin intermediate dimension from %d to %d. "
            "Gate/up shards and down-proj reduction are zero-padded before repack.",
            orig_n,
            padded_n,
        )
    else:
        layer._marlin_moe_n_pad = 0
    ...
    def permute_scales(scales: torch.Tensor, name: str) -> torch.Tensor:
        ...
        if (
            ...
            and scales.nelement() == e * orig_w13_n
        ):
            scales = scales.reshape(e, 1, w13_shards, orig_n)
            scales = torch.nn.functional.pad(scales, (0, n_pad))
            scales = scales.reshape(e, 1, w13_n).contiguous()
        tensor_list = []
        if "w13" in name:
            size_n, size_k = w13_n, k
        else:
